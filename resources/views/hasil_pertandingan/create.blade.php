...
<div class="container-fluid">
    <div class="d-flex align-items-center mb-4">
        ...
        <div>
            <h3 class="fw-bold mb-0" style="color: #0f172a;">Tambah Hasil Pertandingan</h3>
            <p class="text-muted mb-0" style="font-size: 0.9rem;">Input peraih medali emas, perak, dan perunggu per cabang olahraga.</p>
        </div>
    </div>
    ...
    <form action="{{ route('hasil-pertandingan.store') }}" method="POST">
        @csrf
                ...
                <div class="card border-0 shadow-sm h-100" style="border-radius: 16px;">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="nomor_pertandingan" class="form-control" placeholder="Contoh: Tunggal Putra, Lari 100m, Kelas 60 Kg" value="{{ old('nomor_pertandingan') }}">
                    ...
                    <div class="card-body p-4">
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <select name="emas_pelaku_id" id="emas_pelaku_id" class="form-select select2-atlet" onchange="autoFillKontingen(this, 'emas_kontingen')">
                                        ...
                                        @foreach($atlits as $a)
                                            <option value="{{ $a->id }}" data-kontingen="{{ $a->kontingen }}" {{ old('emas_pelaku_id') == $a->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="emas_kontingen" id="emas_kontingen" class="form-select">
                                        ...
                                        @foreach($kotas as $kota)
                                            <option value="{{ $kota->nama }}" {{ old('emas_kontingen') == $kota->nama ? 'selected' : '' }}>
                                                {{ $kota->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <select name="perak_pelaku_id" id="perak_pelaku_id" class="form-select select2-atlet" onchange="autoFillKontingen(this, 'perak_kontingen')">
                                        <option value="">-- Pilih Atlet (Opsional) --</option>
                                        @foreach($atlits as $a)
                                            <option value="{{ $a->id }}" data-kontingen="{{ $a->kontingen }}" {{ old('perak_pelaku_id') == $a->id ? 'selected' : '' }}>
                                                {{ $a->nama }} ({{ $a->cabor ?? 'Atlet' }} - {{ $a->kontingen }})
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="perak_kontingen" id="perak_kontingen" class="form-select">
                                        <option value="">-- Pilih Kontingen / Kota --</option>
                                        @foreach($kotas as $kota)
                                            <option value="{{ $kota->nama }}" {{ old('perak_kontingen') == $kota->nama ? 'selected' : '' }}>
                                                {{ $kota->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <select name="perunggu_pelaku_id" id="perunggu_pelaku_id" class="form-select select2-atlet" onchange="autoFillKontingen(this, 'perunggu_kontingen')">
                                        <option value="">-- Pilih Atlet (Opsional) --</option>
                                        @foreach($atlits as $a)
                                            <option value="{{ $a->id }}" data-kontingen="{{ $a->kontingen }}" {{ old('perunggu_pelaku_id') == $a->id ? 'selected' : '' }}>
                                                {{ $a->nama }} ({{ $a->cabor ?? 'Atlet' }} - {{ $a->kontingen }})
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="perunggu_kontingen" id="perunggu_kontingen" class="form-select">
                                        <option value="">-- Pilih Kontingen / Kota --</option>
                                        @foreach($kotas as $kota)
                                            <option value="{{ $kota->nama }}" {{ old('perunggu_kontingen') == $kota->nama ? 'selected' : '' }}>
                                                {{ $kota->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            ...
                            <button type="submit" class="btn btn-primary px-4" style="border-radius: 8px; font-weight: 600;">
                                <i class="fas fa-save me-2"></i> Simpan Hasil Pertandingan
                            </button>
                        </div>
